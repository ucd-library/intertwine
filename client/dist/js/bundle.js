 ...
 * subject to an additional IP rights grant found at
 * http://polymer.github.io/PATENTS.txt
 */document.head.appendChild(Pt.content);const Ot=new WeakMap;o(t=>e=>{if(!(e instanceof P))throw new Error("unsafeHTML can only be used in text bindings");const i=Ot.get(e);if(void 0!==i&&z(t)&&t===i.value&&e.value===i.fragment)return;const n=document.createElement("template");n.innerHTML=t;const o=document.importNode(n.content,!0);e.setValue(o),Ot.set(e,{value:t,fragment:o})});function Tt(){return V`
  ${ct}
  <style>
    :host {
      display: block;
            ...
            ${this.hasConnections?V`
              ...
              ${this.connections.map(t=>V`
                <div class="dot-list">
                  ...
                  <div>
                    <a class="internal" href="/map/${this.moment}/connection/${t.id}">
                      <strong><em>${t.connection}</em></strong>&nbsp;${t.name}
                    </a>
                  </div>
                </div>
              `)}
...
part of the polymer project is also subject to an additional IP rights grant
found at http://polymer.github.io/PATENTS.txt
*/const At=Lt.a`<iron-iconset-svg name="icons" size="24">
<svg><defs>
...
`}customElements.define("app-map-info-panel",It);function jt(){return V`
    <style>
      :host {
        display: block;
        height: 100%;
...
`}customElements.define("app-about",class extends ht{static get properties(){return{}}constructor(){super(),this.render=Nt.bind(this)}});let Rt=i(27);class Vt extends(Mixin(ht).with(LitCorkUtils)){static get properties(){return{moments:{type:Array},moment:{type:String}}}constructor(){super(),this.render=jt.bind(this),this.moments=[],this.moment="",this.loadMoments(),this._injectModel("AppStateModel","MomentModel")}loadMoments(t){if(t)this.moments.push(t),this.requestUpdate();else for(let t in APP_CONFIG.moments)void 0===Rt.moments[APP_CONFIG.moments[t]]?this.moment=APP_CONFIG.moments[t]:this.moments.push(Rt.moments[APP_CONFIG.moments[t]])}async firstUpdated(t){let e=await this.MomentModel.get(this.moment),i=e.payload.graph.story;i.entrypoint.thumbnail=APP_CONFIG.endpoint+"/"+this.moment+"/"+i.entrypoint.thumbnail,this.loadMoments(e.payload.graph.story)}_onReadStoryClick(t){this.AppStateModel.setLocation("/story/"+t)}_onExploreMapClick(t){this.AppStateModel.setLocation("/map/"+t)}}customElements.define("app-home",Vt);
/**
 * @license
 * Copyright (c) 2017 The Polymer Project Authors. All rights reserved.
 * This code may only be used under the BSD style license found at
 ...
 * http://polymer.github.io/PATENTS.txt
 */
const Bt=(t,e)=>{const i=t.startNode.parentNode,n=void 0===e?t.endNode:e.startNode,o=i.insertBefore(v(),n);i.insertBefore(v(),n);const r=new P(t.options);return r.insertAfterNode(o),r},Dt=(t,e)=>(t.setValue(e),t.commit(),t),Zt=(t,e,i)=>{const n=t.startNode.parentNode,o=i?i.startNode:t.endNode,r=e.endNode.nextSibling;r!==o&&a(n,e.startNode,r,o)},Ft=t=>{l(t.startNode.parentNode,t.startNode,t.endNode.nextSibling)},Ut=(t,e,i)=>{const n=new Map;for(let o=e;o<=i;o++)n.set(t[o],o);return n},qt=new WeakMap,$t=new WeakMap,Gt=o((t,e,i)=>{let n;return void 0===i?i=e:void 0!==e&&(n=e),e=>{if(!(e instanceof P))throw new Error("repeat can only be used in text bindings");const o=qt.get(e)||[],r=$t.get(e)||[],s=[],a=[],l=[];let h,c,d=0;for(const e of t)l[d]=n?n(e,d):d,a[d]=i(e,d),d++;let p=0,u=o.length-1,f=0,m=a.length-1;for(;p<=u&&f<=m;)if(null===o[p])p++;else if(null===o[u])u--;else if(r[p]===l[f])s[f]=Dt(o[p],a[f]),p++,f++;else if(r[u]===l[m])s[m]=Dt(o[u],a[m]),u--,m--;else if(r[p]===l[m])s[m]=Dt(o[p],a[m]),Zt(e,o[p],s[m+1]),p++,m--;else if(r[u]===l[f])s[f]=Dt(o[u],a[f]),Zt(e,o[u],o[p]),u--,f++;else if(void 0===h&&(h=Ut(l,f,m),c=Ut(r,p,u)),h.has(r[p]))if(h.has(r[u])){const t=c.get(l[f]),i=void 0!==t?o[t]:null;if(null===i){const t=Bt(e,o[p]);Dt(t,a[f]),s[f]=t}else s[f]=Dt(i,a[f]),Zt(e,i,o[p]),o[t]=null;f++}else Ft(o[u]),u--;else Ft(o[p]),p++;for(;f<=m;){const t=Bt(e,s[m+1]);Dt(t,a[f]),s[f++]=t}for(;p<=u;){const t=o[p++];null!==t&&Ft(t)}qt.set(e,s),$t.set(e,l)}});function Wt(){return V`
  ${ct}
  <style>
    .floating-btn {
      position: absolute;
  ...
  `}function Jt(){return V`
    ...
    <div id="container" class="container" role="main">      
      ...
      ${this.story.triptych&&this.story.triptych["@id"]?V`
          <div class="triptych">
          ${Gt(this.story.triptych&&this.story.triptych.image,t=>V`            
...
`}customElements.define("app-story-float-btn",class extends ht{static get properties(){return{}}constructor(){super(),this.render=Wt.bind(this)}});let Yt=i(27);class Kt extends(Mixin(ht).with(LitCorkUtils)){static get properties(){return{endpoint:{type:String},headerImgUrl:{type:String},moment:{type:String},paragraphs:{type:Array},story:{type:Object},sources:{type:Array},title:{type:String},jsonData:{type:Object}}}constructor(){super(),this.render=Jt.bind(this),this.endpoint=APP_CONFIG.endpoint,this.moment="",this.headerImgUrl="",this.paragraphs=[],this.story={},this.sources=[],this.paragraphs=[],this.title="",this.jsonData=Yt,this._injectModel("AppStateModel","MomentModel")}async firstUpdated(){this.container=this.shadowRoot.querySelector("#container"),window.IntersectionObserver||(console.log("Injecting IntersectionObserver polyfill"),await i.e(0).then(i.t.bind(null,82,7))),this._initIntersectionObserver()}async _onAppStateUpdate(t){this.container.scrollTo(0,0),this.moment=t.moment;let e=await this.MomentModel.get(this.moment);this.renderStory(e.payload)}renderStory(t){if(t){if(0!==Object.keys(t.graph.story).length){this.story=t.graph.story,this.title=t.graph.story.entrypoint.name,null===t.graph.story.entrypoint.thumbnail.match(/^https?:\/\//g)?this.headerImgUrl=this.endpoint+"/"+this.moment+"/"+t.graph.story.entrypoint.thumbnail:this.headerImgUrl=t.graph.story.entrypoint.thumbnail,this.sources=[];for(let t in this.story.sources.publication){let e=this.story.sources.publication[t].split(/\[|\]/),i={text:e[0],link:e[1],href:e[2]?e[2].replace(/\(|\)/,""):""};this.sources.push(i)}}this.jsonData.moments[this.moment]&&(this.story=this.jsonData.moments[this.moment],this.title=this.story.entrypoint.name,this.headerImgUrl=this.story.entrypoint.thumbnail,this.sources=this.story.sources),this.paragraphs=[];for(let t in this.story)t.includes("paragraph")&&!t.includes("1")&&this.paragraphs.push(this.story[t])}}_initIntersectionObserver(){this.floatBtn=this.shadowRoot.getElementById("floatBtn"),this.footer=this.shadowRoot.querySelector("footer");new window.IntersectionObserver(t=>{t[0].isIntersecting?this.floatBtn.classList.add("hidden"):this.floatBtn.classList.remove("hidden")}).observe(this.footer)}_launchMap(){this.AppStateModel.setLocation("/map/"+this.moment)}}customElements.define("app-story",Kt);let Xt=i(27);class Qt extends(Mixin(ht).with(LitCorkUtils)){static get properties(){return{baseUrl:{type:String},subtitle:{type:String},jsonData:{type:Object},moments:{type:Array}}}constructor(){super(),this.baseUrl=window.location.protocol+"//"+window.location.host,this.subtitle="",this.jsonData=Xt,this.moments=[],this.render=kt.bind(this),this._injectModel("AppStateModel","MomentModel")}async _onAppStateUpdate(t){if("home"===t.page||"about"===t.page)this.subtitle="California's Modern Wine Network";else{this.moment=t.moment;let e=await this.MomentModel.get(this.moment);this._setSubtitle(e.payload)}}_setSubtitle(t){0!==Object.keys(t.graph.story).length?this.subtitle=t.graph.story.entrypoint.name:this.subtitle=this.jsonData.moments[this.moment].entrypoint.name}_onAboutClick(){this.AppStateModel.setLocation("/about/")}}function te(){return V`
  <style>
    :host {
      display: block;
      position: absolute;
...
`}customElements.define("app-header",Qt);var ee=i(42),ie=i.n(ee),ne=i(43),oe=i.n(ne),re=i(44),se=i.n(re);function ae(){return V`
  <style>
    ${ie.a}
    ${oe.a}
    ${se.a}
