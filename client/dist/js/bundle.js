 ...
 * subject to an additional IP rights grant found at
 * http://polymer.github.io/PATENTS.txt
 */const A=new class{handleAttributeExpressions(t,e,i,n){const o=e[0];if("."===o){return new P(t,e.slice(1),i).parts}return"@"===o?[new E(t,e.slice(1),n.eventContext)]:"?"===o?[new M(t,e.slice(1),i)]:new z(t,e,i).parts}handleTextExpression(t){return new S(t)}};
/**
 * @license
 * Copyright (c) 2017 The Polymer Project Authors. All rights reserved.
 * This code may only be used under the BSD style license found at
...
found at http://polymer.github.io/PATENTS.txt
*/
class zt{constructor(t){zt[" "](t),this.type=t&&t.type||"default",this.key=t&&t.key,t&&"value"in t&&(this.value=t.value)}get value(){var t=this.type,e=this.key;if(t&&e)return zt.types[t]&&zt.types[t][e]}set value(t){var e=this.type,i=this.key;e&&i&&(e=zt.types[e]=zt.types[e]||{},null==t?delete e[i]:e[i]=t)}get list(){if(this.type){var t=zt.types[this.type];return t?Object.keys(t).map((function(t){return Ct[this.type][t]}),this):[]}}byKey(t){return this.key=t,this.value}}zt[" "]=function(){},zt.types={};var Ct=zt.types;Object(bt.a)({is:"iron-meta",properties:{type:{type:String,value:"default"},key:{type:String},value:{type:String,notify:!0},self:{type:Boolean,observer:"_selfChanged"},__meta:{type:Boolean,computed:"__computeMeta(type, key, value)"}},hostAttributes:{hidden:!0},__computeMeta:function(t,e,i){var n=new zt({type:t,key:e});return void 0!==i&&i!==n.value?n.value=i:this.value!==n.value&&(this.value=n.value),n},get list(){return this.__meta&&this.__meta.list},_selfChanged:function(t){t&&(this.value=this)},byKey:function(t){return new zt({type:this.type,key:t}).value}}),
/**
@license
Copyright (c) 2015 The Polymer Project Authors. All rights reserved.
This code may only be used under the BSD style license found at
...
</iron-iconset-svg>`;function Mt(){return j`
  <style>
    :host {
      display: block;
      width: 100%;
    ...
    <div ?hidden="${this.isMoment}">
      <div ?hidden="${this.isLink}">
        <div id="singleImage" class="image" type="${this.type}">
          ${"cluster"===this.type?j`<iron-icon ?hidden="${this.thumbnail}" class="cluster-icon" type="${this.type}" icon="intert-wine-icons:cluster-colored"></iron-icon>`:j`<iron-icon ?hidden="${this.thumbnail}" class="type-color" type="${this.type}" icon="intert-wine-icons:${this.type}"></iron-icon>`}
        </div>
      </div>
              ...
              ${this.connections.map(t=>j`
                <div class="dot-list">
                  ...
                  <div>
                    ...
                    ${t.link.formattedConnection?j`${kt(t.link.formattedConnection)}`:j`<strong><em>${t.link.name}</em></strong>&nbsp;${t.node.name}`}
                    </a>
                  </div>
                </div>
              `)}
...
`}customElements.define("app-map-info-panel",Ht);function Nt(){return j`
    <style>
      :host {
        display: block;
        height: 100%;
...
`}customElements.define("app-about",class extends at{static get properties(){return{active:{type:Boolean}}}constructor(){super(),this.render=It.bind(this),this.active=!0}});let jt=i(26);class Rt extends(Mixin(at).with(LitCorkUtils)){static get properties(){return{active:{type:Boolean},moments:{type:Array}}}constructor(){super(),this.render=Nt.bind(this),this.active=!0,this.moments=[],this.loadMoments(),this._injectModel("AppStateModel","MomentModel")}loadMoments(){for(let t in jt.moments)this.moments.push(jt.moments[t])}_onReadStoryClick(t){this.AppStateModel.setLocation("/story/"+t)}_onExploreMapClick(t){this.AppStateModel.setLocation("/map/"+t)}}function Vt(){return j`
  ${lt}
  <style>
    .floating-btn {
      position: absolute;
  ...
  `}customElements.define("app-home",Rt);function Bt(){return j`
    ...
    <div id="container" class="container" role="main">
      ...
      <section class="text-blocks bg-white">
        ...
        ${this.story.text1.paragraphs.map(t=>j`<p>${t}</p>`)}
      </section>
      ...
      <section class="text-image-pairing bg-tan">
        ...
        <div class="text-blocks">
          ...
          ${this.story.text2.paragraphs.map(t=>j`<p>${t}</p>`)}
        </div>
      </section>

      <section class="text-blocks bg-white">
        <h1 ?hidden="${!this.story.text3.title}">${this.story.text3.title}</h1>
        ${this.story.text3.paragraphs.map(t=>j`<p>${t}</p>`)}
      </section>

      <div class="triptych">
        ${this.story.triptych.map(t=>j`
          <div style="background-image: url('../../images/${t.src}');"><span>${t.title}</span></div>
        `)}
      </div>
      ...
      ${this.story.text4?j`
        <section class="text-blocks bg-white">
          ...
          ${this.story.text4.paragraphs.map(t=>j`<p>${t}</p>`)}
        </section>
        `:j`<div class="spacer"></div>`}

      ${this.story.text5?j`
        <section class="text-image-pairing bg-tan">
          <div class="text-blocks">
            <h1>${this.story.text5.title}</h1>
            ${this.story.text5.paragraphs.map(t=>j`<p>${t}</p>`)}
          </div>
...
`}customElements.define("app-story-float-btn",class extends at{static get properties(){return{active:{type:Boolean}}}constructor(){super(),this.render=Vt.bind(this),this.active=!0}});let Dt=i(26);class Zt extends(Mixin(at).with(LitCorkUtils)){static get properties(){return{active:{type:Boolean},selectedMoment:{type:String},story:{type:Object}}}constructor(){super(),this.render=Bt.bind(this),this.active=!0,this._injectModel("AppStateModel")}_onAppStateUpdate(t){this.selectedMomentName=t.moment,this.story=Dt.moments[this.selectedMomentName]}async firstUpdated(){window.IntersectionObserver||(console.log("Injecting IntersectionObserver polyfill"),await i.e(0).then(i.t.bind(null,81,7))),this._initIntersectionObserver()}_initIntersectionObserver(){this.floatBtn=this.shadowRoot.getElementById("floatBtn"),this.footer=this.shadowRoot.querySelector("footer");new window.IntersectionObserver(t=>{t[0].isIntersecting?(this.floatBtn.style.visibility="hidden",this.floatBtn.style.opacity=0,this.floatBtn.style.transition="visibility 0s .5s, opacity .5s ease"):(this.floatBtn.style.visibility="visible",this.floatBtn.style.opacity=1)}).observe(this.footer)}_launchMap(){this.AppStateModel.setLocation("/map/"+this.selectedMomentName)}}customElements.define("app-story",Zt);let Ft=i(26);class Ut extends(Mixin(at).with(LitCorkUtils)){static get properties(){return{baseUrl:{type:String},subtitle:{type:String}}}constructor(){super(),this.baseUrl=window.location.protocol+"//"+window.location.host,this.subtitle="",this.render=Mt.bind(this),this._injectModel("AppStateModel")}_onAppStateUpdate(t){if("home"===t.page)this.subtitle="California's Modern Wine Network";else for(let e in Ft.moments)t.moment===e&&(this.subtitle=Ft.moments[e].title)}_onAboutClick(){this.AppStateModel.setLocation("/about/")}}function qt(){return j`
  <style>
    :host {
      display: block;
      position: absolute;
...
`}customElements.define("app-header",Ut);var $t=i(41),Gt=i.n($t),Wt=i(42),Jt=i.n(Wt),Yt=i(43),Kt=i.n(Yt);function Xt(){return j`
  <style>
    ${Gt.a}
    ${Jt.a}
    ${Kt.a}
