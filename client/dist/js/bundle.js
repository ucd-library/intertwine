...
`}customElements.define("app-moment-float-btn",class extends at{static get properties(){return{active:{type:Boolean}}}constructor(){super(),this.render=Vt.bind(this),this.active=!0}});class Zt extends(Mixin(at).with(LitCorkUtils)){static get properties(){return{active:{type:Boolean},moment:{type:String}}}constructor(){super(),this.render=Dt.bind(this),this.active=!0,this._injectModel("AppStateModel")}async firstUpdated(){let t=await this.AppStateModel.get();this.moment=t.moment,window.IntersectionObserver||(console.log("Injecting IntersectionObserver polyfill"),await i.e(0).then(i.t.bind(null,80,7))),this._initIntersectionObserver()}_initIntersectionObserver(){window,this.floatBtn=this.shadowRoot.getElementById("floatBtn"),this.footer=this.shadowRoot.querySelector("footer");new window.IntersectionObserver(t=>{t[0].isIntersecting?(this.floatBtn.style.visibility="hidden",this.floatBtn.style.opacity=0,this.floatBtn.style.transition="visibility 0s .5s, opacity .5s ease"):(this.floatBtn.style.visibility="visible",this.floatBtn.style.opacity=1)}).observe(this.footer)}_launchMap(){this.AppStateModel.setLocation("/map/"+this.moment)}}function Ft(){return j`
    ${lt}
    <link rel="stylesheet" type="text/css" href="../elements/styles/custom_styles.css">
    <style>
      :host {
...
`}customElements.define("app-moment-chardonnay",Zt);class Ut extends(Mixin(at).with(LitCorkUtils)){static get properties(){return{active:{type:Boolean},moment:{type:String}}}constructor(){super(),this.render=Ft.bind(this),this.active=!0,this._injectModel("AppStateModel")}async firstUpdated(){let t=await this.AppStateModel.get();this.moment=t.moment,window.IntersectionObserver||(console.log("Injecting IntersectionObserver polyfill"),await i.e(0).then(i.t.bind(null,80,7))),this._initIntersectionObserver()}_initIntersectionObserver(){this.floatBtn=this.shadowRoot.getElementById("floatBtn"),this.footer=this.shadowRoot.querySelector("footer");new window.IntersectionObserver(t=>{t[0].isIntersecting?(this.floatBtn.style.visibility="hidden",this.floatBtn.style.opacity=0,this.floatBtn.style.transition="visibility 0s .5s, opacity .5s ease"):(this.floatBtn.style.visibility="visible",this.floatBtn.style.opacity=1)}).observe(this.footer)}_launchMap(){this.AppStateModel.setLocation("/map/"+this.moment)}}customElements.define("app-moment-jop",Ut);class qt extends(Mixin(at).with(LitCorkUtils)){static get properties(){return{active:{type:Boolean},selectedMoment:{type:String}}}constructor(){super(),this.render=Bt.bind(this),this.active=!0,this._injectModel("AppStateModel")}_onAppStateUpdate(t){this.selectedMoment=t.moment}}customElements.define("app-moment",qt);class Gt extends(Mixin(at).with(LitCorkUtils)){static get properties(){return{baseUrl:{type:String},currentTopic:{type:String}}}constructor(){super(),this.render=St.bind(this),this.baseUrl=window.location.protocol+"//"+window.location.host,this._injectModel("AppStateModel")}async firstUpdated(){let t=await this.AppStateModel.get();this.currentTopic=t.moment}_onAboutClick(){this.AppStateModel.setLocation("/about/")}}function Wt(){return j`
...
`}customElements.define("app-header",Gt);var $t=i(40),Jt=i.n($t),Yt=i(41),Kt=i.n(Yt),Xt=i(42),Qt=i.n(Xt);function te(){return j`
